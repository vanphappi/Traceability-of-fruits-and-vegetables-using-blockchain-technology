                  ...
                  <div class="row">
                    <div class="col-lg-6">
                      <div class="form-group">
                        <label class="form-control-label" for="input-tensanpham">Tên sản phẩm</label>
                        <input  type="text" id="input-tensanpham" class="form-control" placeholder="VD: cà rốt">
                      </div>
                    </div>
                    <div class="col-lg-6">
                      <div class="form-group">
                        <label class="form-control-label" for="input-ngaytao">Ngày tạo</label>
                        <input  type="date" id="input-ngaytao" class="form-control" placeholder="">
                      </div>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                        <label class="form-control-label" for="input-diachi">Địa chỉ</label>
                        <input id="input-diachi" class="form-control" placeholder="VD: Thành phố Thủ Đức" type="text">
                      </div>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                        ...
                        <textarea id="input-motasanpham" rows="4" class="form-control" placeholder="Viết mô tả cho sản phẩm"></textarea>
                      </div>
                    </div>
                  </div>
                  <div class="row">
  ...
  <script>
    ...
    if (typeof web3 !== 'undefined') {
      ...
      $("#input-luuthongtin").on("click", function(){
        ...
        then(data=>{
        var account=data[0].nhaphanphoi;
        var contract = new web3.eth.Contract(contractAbi, contractAddress);
        var tensanpham = $('#input-tensanpham').val();
        var ngaynhap = $('#input-ngaytao').val();
        var mota =  $('#input-motasanpham').val();
        var diachi=  $('#input-diachi').val();
        web3.eth.getAccounts().then(async function() {
        var receipt = await contract.methods.themsanpham(tensanpham,ngaynhap,mota,diachi).send({ from:account, gas: 1000000 })
          .on('receipt',function(receipt){
            idsanpham = receipt.events.Added.returnValues[0];
            $('#msg').text('ID sản phẩm vừa tạo là : '+idsanpham);
          });
        });
